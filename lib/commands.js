...
bedrock.events.on('bedrock-cli.init', () => {
  ...
  bedrock.program.option('--watch <mode>',
    'Watch resources to trigger optimize run (true, false) [false].',
    /^(true|false)$/i, 'false');
  bedrock.events.emit('bedrock-views.cli.watch.configure', bedrock.program);
});
...
function _optimizeJS(callback) {
  ...
  async.auto({
    ...
    optimize: ['templates', 'buildImportAll', (results, callback) => {
      ...
      if(bedrock.program.watch === 'true') {
        bedrock.events.emit('bedrock-views.watch.run', options, callback);
      } else {
    ...
    report: ['optimize', (results, callback) => {
      const time = Date.now() - start;
      logger.info('JS optimization complete in ' + time + 'ms.');
      callback();
    }]
  }, err => {
    if(err) {
