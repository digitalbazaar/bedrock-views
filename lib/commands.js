...
const brWebpack = require('bedrock-webpack');
...
bedrock.events.on('bedrock-cli.init', () => {
  ...
  bedrock.events.emit('bedrock-views.cli.optimize.init', command);
});
...
function _compileLess(callback) {
  logger.info('[bedrock-views] Compiling less and combining css from ' +
    'browser packages...');
  ...
  if(Object.keys(pkgs).length === 0) {
    // nothing to compile
    logger.info('[bedrock-views] No browser packages found.');
    return callback();
  }
  ...
  if(_fileOpts.length === 0) {
    logger.info('[bedrock-views] No less or css found.');
    return callback();
  }
  ...
  logger.info('[bedrock-views] Compiling less imports:\n' + src);
  ...
  async.auto({
    ...
    report: ['stat', (results, callback) => {
      ...
      logger.info(
        '[bedrock-views] Less compilation complete (' + bytes + ') in ' +
        time + 'ms. Written to: ' + cfg.out);
      callback();
    }]
  }, function(err) {
    if(err) {
      logger.error('[bedrock-views] Less compilation failed', err);
    }
    callback(err);
  });
...
function _optimizeCSS(callback) {
  ...
  logger.info('[bedrock-views] Optimizing CSS...');

  async.auto({
    compile: callback => _compileLess(callback),
    read: ['compile', (results, callback) => {
      logger.info('[bedrock-views] Minifying CSS...');
      fs.readFile(
    ...
    report: ['minify', (results, callback) => {
      ...
      const time = Date.now() - start;
      logger.info('[bedrock-views] CSS minification complete (' +
    ...
    if(err) {
      logger.error('[bedrock-views] CSS optimization failed', err);
    } else {
      logger.info('[bedrock-views] CSS optimization complete.');
    }
    callback(err);
  });
}
...
function _optimizeJS(callback) {
  ...
  logger.info('[bedrock-views] Optimizing JavaScript...');

  async.auto({
    templates: callback => _optimizeAngularTemplates(callback),
    buildImportAll: callback => packages.buildImportAllModule(callback),
    buildAliases: callback => _buildAliases(callback),
    webpack: ['templates', 'buildImportAll', 'buildAliases',
      (results, callback) => {
      brWebpack.optimize({
        main: [
          bedrock.config.views.angular.optimize.templates.out,
          bedrock.config.views.system.paths.importAll
        ],
        configs: results.buildAliases
      }, callback);
    }],
    report: ['webpack', (results, callback) => {
      const outSize =
        fs.statSync(bedrock.config['bedrock-webpack'].out).size;
      const time = Date.now() - start;
      logger.info('[bedrock-views] JS minification complete (' +
        _byteSizeToString(outSize, 2) + ') in ' + time + 'ms. Written to: ' +
        bedrock.config.views.css.optimize.out);
      callback();
    }]
  }, err => {
    if(err) {
      logger.error('[bedrock-views] JavaScript optimization failed', err);
    } else {
      logger.info('[bedrock-views] JavaScript optimization complete.');
    }
    callback(err);
  });
}
...
function _ngAnnotate(moduleName, path, contents) {
  ...
  if(result.errors) {
    logger.error('[bedrock-views] ng-annotate failed for ' +
...
function _optimizeAngularTemplates(callback) {
  logger.info('[bedrock-views] Angular template optimizer running...');
  ...
  async.auto({
    ...
    mkdirp: callback => mkdirp.mkdirp(path.dirname(cfg.out), callback),
    prefix: ['mkdirp', (results, callback) => {
      const data = [
    ...
    optimize: ['get', 'prefix', (results, callback) => {
      logger.info('[bedrock-views] Minifying angular templates...');
      try {
    ...
    report: ['suffix', (results, callback) => {
      ...
      const time = Date.now() - start;
      logger.info('[bedrock-views] Angular template optimization complete (' +
        _byteSizeToString(inSize, 2) + ' => ' + _byteSizeToString(outSize, 2) +
...
function _optimizeTemplate(pkgCfg, file) {
  ...
  try {
    ...
    logger.warning('[bedrock-views] Could not minify "' + file + '"', e);
  }
...
function _getAngularTemplates(callback) {
  ...
  for(let name in pkgs) {
    ...
    if(ignore.packages.indexOf(name) !== -1) {
      logger.debug(
        '[bedrock-views] Angular template optimizer ignoring package: ' + name);
      continue;
    }
    ...
    if(name in pkgCfgs_) {
      ...
      logger.debug(
        '[bedrock-views] Angular template optimizer ignoring ' +
        'non-angular-based package: ' + name);
      continue;
    }
  ...
  async.auto({
    getFiles: callback => {
      ...
      async.each(pkgCfgs, (pkgCfg, callback) => {
        ...
        if(!err) {
          logger.debug(
            '[bedrock-views] Angular template optimizer including templates ' +
            'matching patterns:\n' + includePatterns.join('\n') + '\n');
          logger.debug(
            '[bedrock-views] Angular template optimizer excluding templates ' +
