...
bedrock.events.on('bedrock-cli.init', () => {
  ...
  bedrock.events.emit('bedrock-cli.compile-less.configure', command);
});
...
bedrock.events.on('bedrock-cli.init', () => {
  ...
  bedrock.events.emit('bedrock-cli.optimize.configure', command);
});
...
function _optimizeJS(callback) {
  ...
  async.auto({
    ...
    optimize: ['templates', 'buildImportAll', (results, callback) => {
      const options = {
        input: [
      ...
      bedrock.events.emit('bedrock.optimize.run', options, callback);
    }],
    report: ['optimize', (results, callback) => {
      const time = Date.now() - start;
      logger.info('[bedrock-views] JS optimization complete in ' +
        time + 'ms.');
      callback();
    }]
  }, err => {
