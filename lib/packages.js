...
api.readPackagesSync = rootModule => {
  ...
  // read root package
  const rootPackage = _readPackageSync({
    moduleName: rootModule
  });
...
function _readPackageSync({moduleName, cache, pseudoPkg, paths = null}) {
  const pkg = {};
...
function _readDependenciesSync(pkg, cache) {
  ...
  for(const dep of deps) {
    ...
    const depPkg = cache[dep] = _readPackageSync({
      moduleName: dep,
      cache,
      paths: [
...
function _getBrowserDependencies(pkg) {
  // `bedrock.browserDependencies` can be specified or default to scanning
  // all dependencies and peerDependencies.
  let deps = _.get(pkg, 'manifest.bedrock.browserDependencies', 'all');
  if(deps === 'all') {
    // special value of `all` means use `pkg.manifest.dependencies` and
    // `pkg.manifest.peerDependencies`
    deps = [
      ...Object.keys(pkg.manifest.dependencies || {}),
      // FIXME: is this needed?  peerdeps should be deps elsewhere
