...
bedrock.events.on('bedrock-express.configure.static', () => {
  // serve minified js
  bedrock.config.express.static.push({
    route: '/system/app/main.min.js',
    path: bedrock.config['bedrock-webpack'].out,
    file: true
  });
});
...
function _buildImportAllModule(callback) {
  ...
  data += 'import {start} from \'bedrock-angular\';\n';
  data += 'start();\n';
    ...
    if(!files) {
      for(let i = 0; i < mainFields.length && !files; ++i) {
        ...
        if(!Array.isArray(files) && typeof files === 'object') {
          logger.warning(
            '[bedrock-views] object "browser" field found: ' + moduleName);
          files = undefined;
        }
      }
    }
...
function _getManifestOverride(pkg) {
