...
function _detectCycle(cache) {
  const initialPkgName = Object.keys(cache)[0];
  const visited = new Set();
  const recStack = {};

  _detectCylceUtil(cache, initialPkgName, visited, recStack);
}

function _detectCylceUtil(cache, name, visited, recStack) {
  if(!visited.has(name)) {
    visited.add(name);
    recStack[name] = true;
    ...
    for(const dep of deps) {
      ...
      if(!visited.has(dep)) {
        _detectCylceUtil(cache, dep, visited, recStack);
      } else if(recStack[dep]) {
        const msg = `Cycle in Depedencies: ${Object.keys(recStack)
          .filter(dep => recStack[dep] === true)}`;
        throw new Error(msg);
      }
    }
  }
