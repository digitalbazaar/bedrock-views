...
bedrock.events.on('bedrock-express.configure.routes', app => {
  // TODO: only add this route when not running in minified/optimized mode
  ...
  app.get(vueCfg.compiler, (req, res, next) => {
    ...
    console.log('compile Vue SFC component:', componentPath);

    let filename;
    try {
      filename = api.moduleUrlToFilename(componentPath);
    } catch(e) {
      return next(e);
    }

    vueSfcCompiler(filename, (err, result) => {
      if(err) {
...
api.moduleUrlToFilename = url => {
  ...
  throw new BedrockError(
    'Not Found.', 'NotFound', {'public': true, httpStatusCode: 400});
};
