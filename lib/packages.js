...
function _sortPackages(cache) {
  ...
  const cyclicDetectionThreshold = 10;
  let maxCyclicCount = 0;
  const cyclicDetector = {};

  for(let i = 0; i < unsorted.length; i++) {
    const pkg = unsorted[i];
    const key = `${pkg.manifest.name}@${pkg.manifest.version}`;
    cyclicDetector[key] = {count: 0, dependencies: pkg.manifest.dependencies};
  }
  ...
  while(unsorted.length > 0) {
    ...
    if(add) {
      ...
      const key = `${pkg.manifest.name}@${pkg.manifest.version}`;
      cyclicDetector[key].count += 1;
      const count = cyclicDetector[key].count;
      maxCyclicCount = count >= maxCyclicCount ? count : maxCyclicCount;
      unsorted.push(pkg);
    }

    if(maxCyclicCount >= cyclicDetectionThreshold) {
      const badPackages = Object.keys(cyclicDetector).filter(key =>
        (cyclicDetectionThreshold - cyclicDetector[key].count) <= 3)
        .map(name => ({
          name,
          dependencies: cyclicDetector[name].dependencies
        }));
      logger.error('Found a cyclic dependency, look at these dependencies: ' +
        `${JSON.stringify(badPackages, null, 2)}`);
      process.exit(1);
    }
  }
