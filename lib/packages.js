...
function _sortPackages(cache) {
  ...
  while(unsorted.length > 0) {
    ...
    if(cyclicHashSet[JSON.stringify(unsorted)] === true) {
      throw new Error(`Cyclic Dependency Detected: ${unsorted
        .map(dep => dep.manifest.name)}`);
    } else {
      cyclicHashSet[JSON.stringify(unsorted)] = true;
    }
