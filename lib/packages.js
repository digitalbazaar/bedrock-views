...
api.readPackagesSync = rootModule => {
  ...
  // attempting to reading any manually configured dependencies from disk)
...
bedrock.events.on('bedrock-express.configure.static', callback => {
  try {
    ...
    for(const name in pkgs) {
      ...
      // FIXME: fix and uncomment, currently causes infinite fetch loop on
      // frontend
...
function _sortPackages(cache) {
  ...
  const sorted = Object.keys(cache).map(name => cache[name]);
  sorted.sort((a, b) => _isParent(a, b) ? 1 : (_isParent(b, a) ? -1 : 0));
...
function _getBrowserDependencies(pkg) {
  // prefer dependencies from `bedrock.browserDependencies` but allow
  // `dependencies` if not `bedrock` is not present
  return pkg.manifest.bedrock ?
    pkg.manifest.bedrock.browserDependencies || [] :
    [];
    // FIXME: uncomment, currently pulls in backend dependencies
    //(Object.keys(pkg.manifest.dependencies || {}));
}

function _isParent(pkg, child) {
  while(child.parent) {
    if(child.parent === pkg) {
      return true;
    }
