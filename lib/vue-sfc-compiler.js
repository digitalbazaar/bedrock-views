...
function enqueue(info, componentPath, callback) {
...
function taskHandler(task, callback) {
  ...
  */
  ...
  async.auto({
    compile: callback => compile(task.filename, task.pkg, callback),
...
function compile(fixture, pkg, callback) {
...
function _getExternals(pkg) {
  const {manifest: {dependencies = {}}} = pkg;
  const ret = {};
