...
function compile({filename, pkg, externals}, callback) {
  // compute peer dependencies `node_modules` dir
  const peerNodeModulesDir = module.parent.filename.substr(
    0, module.parent.filename.indexOf('/bedrock-views'));
  const compiler = webpack({
    ...
    externals,
    /*
      Enable loading of peer dependencies.
    */
    resolve: {
      modules: [peerNodeModulesDir, 'node_modules']
    },
    module: {
      rules: [{
        test: /\.less$/,
