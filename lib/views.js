...
function _optimizeTemplate(pkgCfg, file) {
  ...
  var url = path.join(
    componentsUrl, path.basename(pkgCfg.pkg.path),
    path.relative(pkgCfg.pkg.path, file));
...
function _getAngularTemplates(callback) {
  ...
  for(var name in pkgs) {
    ...
    patterns.forEach(function(pattern) {
      var base = fs.realpathSync(path.resolve(pkg.path));
  ...
  async.auto({
    getFiles: function(callback) {
      ...
      async.forEach(pkgCfgs, function(pkgCfg, callback) {
        var options = {
          base: fs.realpathSync(path.resolve(pkg.path)),
...
function _glob(patterns, options, callback) {
  ...
  async.forEach(patterns, function(pattern, callback) {
    ...
    rval = _.difference(_.uniq(rval), _.uniq(exclusions));
    callback(null, rval);
  });
}
