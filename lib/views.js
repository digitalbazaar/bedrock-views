...
function _getTemplateOverride(pkg, file) {
  ...
  var clientData = bedrock.config.views.vars.clientData;
  var overrides = clientData.angular.templates.overrides;
  var url = _getTemplateUrl(pkg, file);
  if(url in overrides) {
...
function _getTemplateUrl(pkg, file) {
  // FIXME: if optimized, componentsUrl is omitted, if not, it's included
  //var componentsUrl = bedrock.config.requirejs.bower.componentsUrl;
  //url = path.join(componentsUrl, url);

  // URL is only used as an ID in the template cache, due to the inability to
  // access the requirejs config use a simplified path here that only includes
  // the module name so modules can define relative template URLs based on
